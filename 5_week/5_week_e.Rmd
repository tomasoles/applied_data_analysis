...
In previous lectures, we've analyzed text in **tidy** format—one token per document per row, as produced by `unnest_tokens()`.

This structure integrates seamlessly with `dplyr`, `tidyr`, and `ggplot2`, allowing flexible text analysis.

---

### Why Non-Tidy Formats Matter

Most R NLP tools (beyond `tidytext`) use **non-tidy** data formats, such as:

- `DocumentTermMatrix` (from **tm**)
- `dfm` (from **quanteda**)
- Sparse matrices (from **Matrix**)

Bridging these structures is essential for combining tidy workflows with established NLP tools.

> “It is often the quality of the glue that determines the power of the system.”  
> — Hal Abelson
...
A **Document-Term Matrix (DTM)** has:

- Rows = documents  
- Columns = terms  
- Cells = term frequencies  

DTMs are usually sparse matrices.

---

### Converting Between Formats

`tidytext` provides verbs for conversion:

- `tidy()` — DTM → tidy tibble  
- `cast_*()` — tidy tibble → matrix  
...
knitr::include_graphics("img/tmwr_0501.png")
```

---

## Example: `DocumentTermMatrix` (tm)
...
---
### What is inside the document-term matrix?
...
Produces columns: document, term, count.

Notice that the tidy format is no longe *sparse* vector. 

---
### Standard pipeline: sentiment
...
```{r  echo = FALSE, fig.cap = "The terms with the highest tf-idf from each of four selected inaugural addresses.", fig.height=6, fig.width=7,fig.align="center"}
...
```{r  out.width='48%',fig.align="center", message=F}
year_term_counts %>%
...
### From tidy text format to matrix
...
Corpus objects (from tm) store:

Raw text

Metadata (id, date, title, etc.)
